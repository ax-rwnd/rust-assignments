...
fn main () {
    ...
    // Test codgen
    let old_seed: u32 = seed;
    seed = 0x3e944b9f;
    assert!(codgen(&mut seed) == 0x891432f9, "First code invalid");
    assert!(codgen(&mut seed) == 0x4aa1dccc, "Second code invalid");
    assert!(codgen(&mut seed) == 0xc54270fa, "Thirdj code invalid");
    assert!(codgen(&mut seed) == 0x9885155f, "Fourth code invalid");
    assert!(codgen(&mut seed) == 0xce83d1b8, "Fifth code invalid");
    seed = old_seed;
