...
unsafe fn decode(wordarr: &[u32], bytearr: &mut [u8]) -> u32 {
    let m: u32;
    let mut r: u32;
    let x: u32;
    let y: u32;
...
fn main () {
    unsafe {
        ...
        SEED = 0x3e944b9f;
        assert!(codgen() == 0x891432f9, "First code invalid");
        assert!(codgen() == 0x4aa1dccc, "Second code invalid");
        assert!(codgen() == 0xc54270fa, "Thirdj code invalid");
        assert!(codgen() == 0x9885155f, "Fourth code invalid");
        assert!(codgen() == 0xce83d1b8, "Fifth code invalid");
        SEED = old_seed;
    }
