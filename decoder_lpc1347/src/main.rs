...
fn decode(wordarr: &[u32], bytearr: &mut [u8], mut seed: &mut u32) -> u32 {
    ...
    if wordarr[0] == 0u32 {
        ...
        r = (-(codgen(&mut seed) as i32)) as u32;
...
fn main () {
    ...
    interrupt::free(|cs| {
        let ctx = DWT.borrow(cs);
        unsafe { ctx.cyccnt.write(0); }
        ctx.enable_cycle_counter();

        let start: u32 = ctx.cyccnt.read();
        decode(&abc, &mut plain, &mut seed);
        let stop: u32 = ctx.cyccnt.read();

        let _ = writeln!(stdout, "decoded: {:?}", from_utf8(&plain).unwrap());
        let _ = writeln!(stdout, "cycles: {}", stop-start);
    });
}
