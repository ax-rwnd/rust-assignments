...
fn format_log(command_log: &Vec<GameInput>) -> String {
    ...
    if command_log.len() < 4 {
        return command_log[0..command_log.len()].iter().fold(format!("tries\t| command\n"),
                |output, input| format!("{}{}\t| {}\n", output, input.0, input.1));
    } else {
        return command_log[max(command_log.len()-4,1)..command_log.len()].iter().fold(format!("tries\t| command\n"),
                |output, input| format!("{}{}\t| {}\n", output, input.0, input.1));
    }
}
